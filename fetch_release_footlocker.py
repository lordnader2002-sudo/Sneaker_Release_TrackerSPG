...
def extract_rows(soup: BeautifulSoup) -> list[dict[str, Any]]:
    ...
    for a in soup.find_all("a", href=True):
        ...
        # Primary: scan preceding siblings of each ancestor — handles the
        # split "Mar / 20" date box that sits beside the product panel.
        d, container = find_sibling_date(a, DATE_RE, default_year)

        # Fallback: blob-search up the ancestor tree (catches pages where the
        # date is embedded in the same container as the product title).
        if d is None:
            container = a.parent
            for _ in range(6):
        ...
        # Labeled-only price from the card container
